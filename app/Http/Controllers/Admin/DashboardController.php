...
class DashboardController extends Controller
{
    public function index(): Response
    {
        ...
        return Inertia::render('Admin/Dashboard', [
            'stats' => [
                ...
                'suscripcionesNuevasHoy' => Suscripcion::activas()->where('created_at', '>=', $hoy)->count(),
            ...
            'eventosProximos' => Evento::where('fecha', '>=', now())
                ->orderBy('fecha')
                ->take(3)
                ->get(['id', 'nombre', 'fecha', 'estado']),

            'gestionUsuarios' => User::latest()->take(5)->get([
                'id', 'nombre', 'apodo', 'email', 'rol', 'estado', 'created_at',
            ]),
            ...
            'accionesRapidas' => [
                ...
                ['label' => 'Crear Evento', 'desc' => 'Organiza un nuevo evento', 'icon' => 'pi-calendar-plus', 'route' => 'admin.eventos.index'],
                ['label' => 'Enviar Invitación', 'desc' => 'Invita usuarios a la plataforma', 'icon' => 'pi-envelope', 'route' => 'admin.invitaciones.index'],
            ],
    ...
    /**
     * Feed simple de actividad combinando usuarios, transacciones y eventos recientes.
     * (Más adelante se puede reemplazar por una tabla real de auditoría.)
     */
    private function actividadReciente(): array
    {
        $usuarios = User::latest()->take(3)->get()->map(fn ($u) => [
            'tipo' => 'usuario_nuevo',
            'icon' => 'pi-user-plus',
        ...
        $pagos = Transaccion::where('estado', 'aprobada')->latest()->take(3)->get()->map(fn ($t) => [
            'tipo' => 'pago',
            'icon' => 'pi-dollar',
            'texto' => 'Pago de $'.number_format($t->monto, 2).' por @'.($t->usuario?->apodo ?? 'usuario'),
            'fecha' => $t->created_at,
        ]);

        $eventos = Evento::latest()->take(2)->get()->map(fn ($e) => [
            'tipo' => 'evento',
            'icon' => 'pi-calendar',
        ...
        return $usuarios->concat($pagos)->concat($eventos)
            ->sortByDesc('fecha')
            ->take(6)
            ->values()
            ->all();
