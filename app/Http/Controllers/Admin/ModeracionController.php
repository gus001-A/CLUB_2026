...
/**
 * Antes se llamaba ReporteController. Se renombró para dejar libre ese
 * nombre para el nuevo módulo de reportes analíticos (PDF/Excel). Esta
 * clase sigue haciendo exactamente lo mismo: moderación de reportes entre
 * usuarios (acoso, perfil falso, spam, etc.) — ahora vive bajo /admin/soporte.
 */
class ModeracionController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $reportes = $query->latest()->paginate(2)->withQueryString();
    ...
    {
        $reporte->update(['estado' => 'revisado', 'atendido_por_admin_id' => Auth::guard('admin')->id()]);
    ...
    {
        $reporte->update(['estado' => 'resuelto', 'atendido_por_admin_id' => Auth::guard('admin')->id()]);
