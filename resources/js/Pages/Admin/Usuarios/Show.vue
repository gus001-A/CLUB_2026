    ...
    <AdminLayout>
        ...
        <div class="w-full max-w-[1920px] mx-auto px-2 sm:px-4">
            <Link :href="route('admin.usuarios.index')"
                class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-brand mb-4">
            ...
            <div class="admin-card overflow-hidden mb-6">
                ...
                <div class="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-4">
                        <div class="rounded-full text-brand flex items-center justify-center shrink-0 font-bold overflow-hidden"
                            style="background:var(--brand-soft);width:64px;height:64px;font-size:1.5rem">
                            <template v-if="fotoPrincipal && fotoPrincipal.url">
                                <img :src="fotoPrincipal.url" class="w-full h-full object-cover"
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-400">
                                @{{ usuario.apodo }}
                                <span v-if="edad(usuario.fecha_nacimiento)"> · {{ edad(usuario.fecha_nacimiento) }}
                                    años</span>
                    ...
                    <div class="flex items-center gap-2 shrink-0 flex-wrap">
                        <span v-if="perfil" class="px-3 py-1.5 rounded-lg text-xs font-semibold text-brand"
                            style="background:var(--brand-soft)">{{ tipoLabel[perfil.tipo] || 'Personal' }}</span>
                        <span class="px-3 py-1.5 rounded-lg text-xs font-semibold capitalize"
                            :class="estadoColores[usuario.estado]">{{ usuario.estado }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-6 items-start w-full">
                    ...
                    <div class="admin-card p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="flex items-center gap-3 bg-gray-50/60 rounded-xl p-3">
                                <div class="rounded-lg text-brand flex items-center justify-center shrink-0"
                                    style="width:40px;height:40px;background:var(--brand-soft)"><i
                                        class="pi pi-envelope text-sm"></i></div>
                                <div>
                            ...
                            <div class="flex items-center gap-3 bg-gray-50/60 rounded-xl p-3">
                                <div class="rounded-lg text-brand flex items-center justify-center shrink-0"
                                    style="width:40px;height:40px;background:var(--brand-soft)"><i
                                        class="pi pi-phone text-sm"></i></div>
                                <div>
                            ...
                            <div class="flex items-center gap-3 bg-gray-50/60 rounded-xl p-3">
                                <div class="rounded-lg text-brand flex items-center justify-center shrink-0"
                                    style="width:40px;height:40px;background:var(--brand-soft)"><i
                                        class="pi pi-map-marker text-sm"></i></div>
                                <div>
                                    <p class="text-[11px] text-gray-400 uppercase font-medium">Ciudad</p>
                                    <p class="text-sm font-semibold text-gray-800">{{ usuario.ciudad ||
                            ...
                            <div class="flex items-center gap-3 bg-gray-50/60 rounded-xl p-3">
                                <div class="rounded-lg text-brand flex items-center justify-center shrink-0"
                                    style="width:40px;height:40px;background:var(--brand-soft)"><i
                                        class="pi pi-calendar text-sm"></i></div>
                                <div>
                            ...
                            <div class="flex items-center gap-3 bg-gray-50/60 rounded-xl p-3">
                                <div class="rounded-lg text-brand flex items-center justify-center shrink-0"
                                    style="width:40px;height:40px;background:var(--brand-soft)"><i
                                        class="pi pi-clock text-sm"></i></div>
                                <div>
                                    <p class="text-[11px] text-gray-400 uppercase font-medium">Código de invitación</p>
                                    <p class="text-sm font-semibold text-gray-800">{{ usuario.codigo_invitacion || '—'
                                        }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 bg-gray-50/60 rounded-xl p-3">
                                <div class="rounded-lg text-brand flex items-center justify-center shrink-0"
                                    style="width:40px;height:40px;background:var(--brand-soft)"><i
                                        class="pi pi-user text-sm"></i></div>
                                <div>
                                    <p class="text-[11px] text-gray-400 uppercase font-medium">Registrado el</p>
                                    <p class="text-sm font-semibold text-gray-800">{{ formatDate(usuario.created_at) }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div v-if="intereses.length || pasatiempos.length" class="admin-card p-6">
                        ...
                        <div v-if="intereses.length" class="mb-4">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <span v-for="(i, idx) in intereses" :key="idx"
                                    class="px-3 py-1 rounded-full text-xs font-medium text-brand"
                                    style="background:var(--brand-soft)">{{ i }}</span>
                            </div>
                        </div>
                        <div v-if="pasatiempos.length">
                            <p class="text-[11px] text-gray-400 uppercase font-medium mb-2">Pasatiempos</p>
                            <div class="flex flex-wrap gap-2">
                                <span v-for="(p, idx) in pasatiempos" :key="idx"
                                    class="px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600">{{ p
                                    }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="usuario.rol === 'creador'" class="admin-card p-6">
                        ...
                        <template v-if="usuario.creador">
                            <div class="flex items-center gap-2 mb-4 flex-wrap">
                                ...
                                <span v-if="usuario.creador.es_premium"
                                    class="px-3 py-1 rounded-full text-xs font-semibold bg-amber-50 text-amber-600 flex items-center gap-1">
                                    <i class="pi pi-crown text-[10px]"></i> Premium
                                </span>
                            </div>
                            <p v-if="usuario.creador.biografia" class="text-sm text-gray-600 mb-4">{{
                                usuario.creador.biografia }}</p>
                            ...
                            <div v-if="usuario.creador.categorias?.length" class="mb-3">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <span v-for="(c, idx) in usuario.creador.categorias" :key="idx"
                                        class="px-3 py-1 rounded-full text-xs font-medium text-brand"
                                        style="background:var(--brand-soft)">{{ c }}</span>
                                </div>
                            </div>
                ...
                <div class="w-full lg:w-1/3 min-w-0 flex flex-col gap-6">
                    ...
                    <div class="admin-card p-6 text-center">
                        <div class="mx-auto mb-3 rounded-full flex items-center justify-center"
                            ...
                            <i class="pi" style="font-size:1.5rem"
                                :class="perfil?.esta_verificado ? 'pi-verified' : 'pi-question-circle'"></i>
                        </div>
                    ...
                    <div v-if="fotos.length" class="admin-card p-6">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <div v-for="(foto, idx) in fotos.slice(0, 6)" :key="foto.id || idx"
                                ...
                                @click="openModal(idx)">
                                <img :src="foto.url" class="w-full h-full object-cover" :alt="`Foto ${idx + 1}`"
                                    @error="manejarErrorImagen(idx)" />
                                <div v-if="foto.es_principal"
                                    class="absolute top-1 right-1 bg-brand text-white text-xs px-1.5 py-0.5 rounded-full">
                                    <i class="pi pi-star text-[8px]"></i>
                                </div>
                                <div v-if="erroresImagenes[idx]"
                                    class="absolute inset-0 flex items-center justify-center bg-gray-200 text-gray-500 text-xs">
                                    Error al cargar
                                </div>
                            </div>
