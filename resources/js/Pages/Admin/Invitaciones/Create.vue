...
// --- Vista previa del código (visual, el real se genera en el servidor al guardar) ---
...
<template>
    ...
    <AdminLayout>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <!-- Columna izquierda: formulario + código -->
            <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="flex items-center gap-1 text-sm font-medium text-gray-700 mb-1.5">
                                    Tipo de invitación *
                                    <i class="pi pi-info-circle text-gray-300 text-xs" title="Registro: acceso a la plataforma. Premium: incluye beneficios exclusivos. Evento: acceso a un evento específico."></i>
                                </label>
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                    <div class="flex items-center gap-3 pb-4 mb-4 border-b border-gray-100">
                        ...
                        <h2 class="font-semibold text-gray-800">Código de invitación</h2>
                    </div>
                    <p class="text-sm text-gray-500 mb-3">Genera un código único que tu invitado usará para registrarse.</p>

                    <div class="flex items-center gap-3 mb-4">
                        <div class="flex-1 border-2 border-dashed border-gray-200 rounded-lg py-3 text-center">
                            <span class="text-xl font-bold tracking-wider text-brand">{{ codigoPreview }}</span>
                        </div>
                        <button type="button" @click="regenerar" class="flex items-center gap-1.5 text-sm text-gray-500 hover:text-brand px-3 py-2 border border-gray-200 rounded-lg shrink-0">
                            <i class="pi pi-refresh text-xs"></i> Regenerar
                        </button>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <button type="button" @click="copiarCodigo" class="flex items-center justify-center gap-2 text-sm text-gray-600 border border-gray-300 rounded-lg py-2.5 hover:bg-gray-50">
                            <i class="pi pi-copy text-xs"></i> Copiar código
                        </button>
                        <button type="button" @click="enviarPorCorreo" class="flex items-center justify-center gap-2 text-sm text-gray-600 border border-gray-300 rounded-lg py-2.5 hover:bg-gray-50">
                            <i class="pi pi-envelope text-xs"></i> Enviar por correo
                        </button>
                        <button type="button" @click="enviarPorWhatsapp" class="flex items-center justify-center gap-2 text-sm text-gray-600 border border-gray-300 rounded-lg py-2.5 hover:bg-gray-50">
                ...
                <div class="flex items-center gap-3">
                    <Link :href="route('admin.invitaciones.index')" class="text-sm text-gray-500 hover:text-gray-700 px-4 py-2.5 border border-gray-300 rounded-lg">
                        Cancelar
                    </Link>
                    <button
                        type="button"
                        ...
                        class="ml-auto bg-brand hover:bg-brand-dark text-white font-medium px-6 py-2.5 rounded-lg text-sm disabled:opacity-50 flex items-center gap-2"
                    >
            ...
            <!-- Columna derecha: resumen + recientes -->
            <div class="space-y-6">
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                    <div class="flex items-center gap-3 pb-4 mb-4 border-b border-gray-100">
                        <div class="rounded-full bg-brand/10 text-brand flex items-center justify-center shrink-0" style="width:32px;height:32px">
                            <i class="pi pi-file text-xs"></i>
                        </div>
                        <h2 class="font-semibold text-gray-800">Resumen de la invitación</h2>
                    </div>
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="font-semibold text-gray-800">Invitaciones recientes</h2>
                        </div>
                        <Link :href="route('admin.invitaciones.index')" class="text-brand text-xs font-medium hover:underline">Ver todas</Link>
                    </div>
                    ...
                    <ul class="space-y-3">
                        <li v-for="inv in invitacionesRecientes" :key="inv.id" class="flex items-center justify-between gap-2">
                            ...
                            <div class="text-right shrink-0">
                                <span class="block text-xs font-medium px-2 py-0.5 rounded-full" :class="estadoColores[inv.estado]">{{ estadoLabel[inv.estado] }}</span>
                                <span class="block text-[11px] text-gray-400 mt-0.5">{{ formatDate(inv.created_at) }}</span>
                            </div>
                        </li>
                        <li v-if="!invitacionesRecientes?.length" class="text-gray-400 text-sm">Aún no hay invitaciones.</li>
                    </ul>
                </div>
            </div>
        </div>
