...
// 🔧 Agregué "solicitudesCreadorPendientes" al default, para el badge del
// nuevo grupo "Creadores". Ajusta la key si tu backend la manda distinto.
const badges = computed(() => page.props.badges || { invitacionesPendientes: 0, reportesPendientes: 0, mensajesSoporteSinLeer: 0, pedidosNuevos: 0, productosStockBajo: 0, solicitudesCreadorPendientes: 0, notificaciones: 0 });
...
const PREFIJO_A_BADGE = {
    ...
    creador: 'solicitudesCreadorPendientes',
};
...
const links = computed(() => [
    ...
    },
    {
        name: 'Creadores',
        icon: 'pi-star',
        badgeKey: 'solicitudesCreadorPendientes',
        children: [
            { name: 'Todos los creadores', route: 'admin.creadores.index' },
            { name: 'Solicitudes pendientes', route: 'admin.creadores.solicitudes' },
            ...(route().current('admin.creadores.show')
                ? [{ name: 'Ver Creador', url: window.location.pathname }]
                : []),
        ],
    },
    {
...
<template>
    <div class="min-h-screen bg-gray-50 flex">
        ...
        <div v-if="sidebarAbierto" @click="sidebarAbierto = false" class="fixed inset-0 bg-black/40 z-40 lg:hidden">
        </div>

        <!--
            Sidebar
            -------------------------------------------------------------
            FIX: antes usaba "lg:static", que en pantallas grandes quitaba
            el "fixed" y convertía el aside en un item normal dentro del
            <div class="min-h-screen flex"> de afuera. Como ese contenedor
            es flex y por defecto estira (align-items: stretch) a sus
            hijos hasta igualar al hermano más alto, el sidebar se
            estiraba hasta la altura de <main>: si la página tenía mucho
            contenido, el botón de "Cerrar sesión" (que va después del
            <nav flex-1>) terminaba empujado muy abajo, fuera de la
            pantalla. Con "lg:sticky lg:top-0 lg:h-screen" el sidebar
            SIEMPRE mide exactamente 100vh y queda pegado arriba mientras
            <main> scrollea por su cuenta, así que el botón de cerrar
            sesión siempre queda al fondo visible de la pantalla, sin
            importar qué tan larga sea la página.
        -->
        <aside
            class="w-64 bg-white border-r border-gray-200 flex flex-col shrink-0 fixed inset-y-0 left-0 z-50 transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0"
            :class="sidebarAbierto ? 'translate-x-0' : '-translate-x-full'">
            <div class="px-6 py-5 flex items-center justify-between gap-2 border-b border-gray-100 shrink-0">
                <div class="flex items-center gap-2">
                    <img src="/images/LOGO.png" alt="Club de Fantasías"
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto min-h-0">
                <template v-for="link in links" :key="link.name">
                    <!-- Link simple -->
                    <Link v-if="!link.children" :href="route(link.route)" @click="cerrarSidebarMovil"
                        ...
                        <span v-if="link.badgeKey ? contadorBadge(link.badgeKey) > 0 : link.badge"
                    ...
                    <div v-else>
                        <button type="button" @click="toggleGrupo(link)"
                            ...
                            <span v-if="link.badgeKey && contadorBadge(link.badgeKey) > 0"
                                class="bg-brand text-white text-[11px] font-semibold rounded-full w-5 h-5 flex items-center justify-center"
                        ...
                        <div v-show="estaAbierto(link)" class="mt-1 ml-4 pl-4 border-l border-gray-100 space-y-0.5">
                            <Link v-for="child in link.children" :key="child.name"
                                :href="child.url || route(child.route)" @click="cerrarSidebarMovil"
                                class="block px-3 py-2 rounded-xl text-sm transition-colors" :class="(child.url || isActive(child.route))
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="admin-topbar px-4 sm:px-8 py-4">
                ...
                <div class="flex items-center gap-3 sm:gap-6 shrink-0">
                    <!-- Notificaciones -->
                    <div class="relative">
                        <button @click="toggleNotificaciones" title="Notificaciones" class="admin-icon-btn"
                            style="width:42px;height:42px;font-size:1.15rem">
                            <i class="pi pi-bell"></i>
                            <span v-if="notificacionesNuevas.length > 0" class="admin-icon-badge text-[10px]"
                        ...
                        <div v-if="notificacionesAbiertas" @click="cerrarNotificaciones" class="fixed inset-0 z-30">
                        </div>
                        ...
                        <Transition name="admin-dropdown">
                            <div v-if="notificacionesAbiertas" class="admin-dropdown" style="width:320px">
                                ...
                                <div v-else class="max-h-80 overflow-y-auto divide-y divide-gray-50">
                                    <Link v-for="n in notificaciones" :key="n.id"
                                        ...
                                        <p class="text-sm font-medium"
                                            :class="esNotificacionVista(n.id) ? 'text-gray-400' : 'text-gray-700'">{{
                                                n.titulo }}
                                        </p>
                    ...
                    <!-- Perfil -->
                    <div class="relative">
                        <button type="button" @click="togglePerfil" class="admin-user-chip"
                            style="padding:0.25rem 0.8rem 0.25rem 0.25rem">
                            <div class="relative shrink-0">
                                <div class="rounded-full bg-brand/10 flex items-center justify-center text-brand overflow-hidden"
                                    style="width:40px;height:40px">
                                    <img v-if="admin?.foto_perfil_url" :src="admin.foto_perfil_url"
                                        class="w-full h-full object-cover" />
                                    <i v-else class="pi pi-user text-lg"></i>
                                </div>
                                <span class="absolute rounded-full flex items-center justify-center text-white"
                            ...
                            <div class="text-sm leading-tight hidden sm:block text-left">
                                <p class="font-bold text-gray-800" style="letter-spacing:-0.01em">{{ admin?.nombre ||
                                    'Administrador' }}</p>
                                <p class="text-brand font-medium" style="font-size:0.65rem">{{ admin?.rol ===
                                    'super_admin' ?
                                    'Super Admin' : 'Admin' }}</p>
                            </div>
                            <i class="pi text-gray-300 text-xs ml-1 hidden sm:inline transition-transform"
                                :class="perfilAbierto ? 'pi-chevron-up' : 'pi-chevron-down'"></i>
                        </button>
                        ...
                        <Transition name="admin-dropdown">
                            <div v-if="perfilAbierto" class="admin-dropdown" style="width:280px">
                                <div class="admin-dropdown-header">
                                    <div class="rounded-full bg-brand/10 flex items-center justify-center text-brand shrink-0 overflow-hidden"
                                    ...
                                    <div class="min-w-0">
                                        <p class="font-bold text-gray-900 text-sm truncate"
                                            style="letter-spacing:-0.01em">{{
                                                admin?.nombre || 'Administrador' }}</p>
                                        <p class="text-xs text-gray-400 mt-0.5">{{ admin?.email }}</p>
                                        <span class="inline-flex items-center gap-1 text-[11px] font-semibold mt-1"
                                            style="color:var(--success)">
                                            <i class="pi pi-check-circle" style="font-size:0.7rem"></i> {{ admin?.rol
                                                ===
                                ...
                                <div class="py-1.5">
                                    <Link :href="route('admin.seguridad.index')" @click="cerrarPerfil"
                                        class="admin-dropdown-item">
                                        <i class="pi pi-user text-sm"></i>
                                        Mi perfil
                                    </Link>
                                    <Link :href="route('admin.configuracion.index')" @click="cerrarPerfil"
                                        class="admin-dropdown-item">
                                ...
                                <div class="border-t py-1.5" style="border-color:var(--line)">
                                    <button type="button" @click="logout"
                                        class="admin-dropdown-item admin-dropdown-item--danger">
